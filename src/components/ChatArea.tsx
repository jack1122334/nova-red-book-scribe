...
export const ChatArea = forwardRef<ChatAreaRef, ChatAreaProps>(({
  ...
  useImperativeHandle(ref, () => ({
    ...
    notifyCardUpdate: (cardId: string, content: string, title?: string) => {
      ...
      console.log('Card update notification added:', {
        cardId,
        title,
        content
      });
    ...
    notifyCardCreate: (cardId: string) => {
      ...
      setPendingSystemMessages(prev => [...prev, systemMessage]);
      console.log('Card create notification added:', {
        cardId
      });
    }
  }));
  ...
  const parseXMLTags = (content: string) => {
    ...
    return content.replace(/<new_xhs_card(?:\s+title="[^"]*")?>[^]*?<\/new_xhs_card>/g, '[新卡片已创建]').replace(/<update_xhs_card\s+card_ref_id="[^"]*">[^]*?<\/update_xhs_card>/g, '[卡片已更新]');
  };
  ...
  return <div className="h-full flex flex-col bg-amber-300">
              ...
              {references.map((ref, index) => <div key={index} className="rounded-lg border border-black/20 p-3 text-sm bg-amber-100">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {editingRemark === index ? <div className="space-y-2">
                          <Input value={editRemarkText} onChange={e => setEditRemarkText(e.target.value)} placeholder="添加备注说明..." className="text-sm" autoFocus />
                          ...
                          <Button variant="ghost" size="sm" onClick={() => startEditingRemark(index, ref.user_remark)} className="h-6 w-6 p-0 text-black/40 hover:text-black/60">
                            <Edit3 className="w-3 h-3" />
                          </Button>
                        </div>}
                      
                      {ref.snippet_content && <div className="mt-2 p-2 bg-black/5 rounded text-xs text-black/60 border-l-2 border-black/20">
                          "{ref.snippet_content.substring(0, 100)}..."
                        </div>}
                    </div>
                    
                    <Button variant="ghost" size="sm" onClick={() => removeReference(index)} className="h-6 w-6 p-0 text-black/40 hover:text-black flex-shrink-0">
                      <X className="w-3 h-3" />
                    </Button>
                  </div>
                </div>)}
        ...
        <div className="flex-1 overflow-auto bg-amber-400">
          {messages.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-black/50 p-8">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 text-black/50">
                      <div className="flex gap-1">
                        <div className="w-2 h-2 bg-black/40 rounded-full animate-bounce" style={{
                  animationDelay: '0ms'
                }} />
                        <div className="w-2 h-2 bg-black/40 rounded-full animate-bounce" style={{
        ...
        <MessageDetailsDialog message={selectedMessage} isOpen={isMessageDialogOpen} onClose={closeMessageDialog} />
      </div>;
});
