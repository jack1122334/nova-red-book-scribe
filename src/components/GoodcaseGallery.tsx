...
export const GoodcaseGallery = () => {
  return <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {goodcases.map((goodcase, index) => <Card key={goodcase.id} className="group cursor-pointer transition-all duration-300 hover:shadow-notion-hover border-gray-200/60 bg-white animate-fade-in" style={{
        animationDelay: `${index * 0.1}s`
            ...
            <CardContent className="p-0">
              <div className={`h-24 bg-gradient-to-br ${goodcase.color} border-b border-gray-100/60 flex items-end justify-start p-6 rounded-t-xl`}>
                <Badge variant="secondary" className="bg-white/90 text-gray-700 font-serif text-xs font-medium border-0 rounded-lg shadow-sm">
                  {goodcase.category}
                </Badge>
              </div>
